...
namespace Eigen {
	namespace internal {
		template<>
		struct functor_traits<aslam_bsplines::BiVector>
		{
...
namespace aslam_bsplines {
	...
	return Eigen::Map< const Eigen::Matrix<double, D, 1> >(&coefficients_(0,i),coefficients_.rows());
      ...
      aslam_sparse_block_matrix::SparseBlockMatrix<  Eigen::MatrixXd> curveQuadraticIntegralSparse(const  Eigen::MatrixXd & W, int derivativeOrder) const;
      aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> curveQuadraticIntegralDiagSparse(const Eigen::VectorXd & Wdiag, int derivativeOrder) const;

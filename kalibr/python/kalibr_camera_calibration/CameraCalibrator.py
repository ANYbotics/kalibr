import sm_python
from sm_python import PlotCollection
from kalibr_common import ConfigReader as cr
import aslam_cv_python as acv
import aslam_cameras_april as acv_april
import aslam_cv_backend_python as acvb
import aslam_backend_python as aopt
import aslam_incremental_calibration_python as ic
...
class CameraGeometry(object):
    ...
    def initGeometryFromObservations(self, observations):
        ...
        if not success:
            sm_python.logError("initialization of focal length for cam with topic {0} failed  ".format(self.dataset.topic))
        ...
        if self.model == acvb.DistortedOmni:
            ...
            if not success:
                sm_python.logError("initialization of intrinsics for cam with topic {0} failed  ".format(self.dataset.topic))
...
class CalibrationTarget(object):
    def __init__(self, target, estimateLandmarks=False):
        ...
        for i in range(0,self.target.size()):
            p_t_dv = aopt.HomogeneousPointDv(sm_python.toHomogeneous(self.target.point(i)));
...
class CalibrationTargetOptimizationProblem(ic.CalibrationOptimizationProblem):        
    ...
    def fromTargetViewObservations(cls, cameras, target, baselines, T_tc_guess, rig_observations, useBlakeZissermanMest=True):
        ...
        sm_python.logDebug("Adding a view with {0} cameras and {1} error terms".format(len(cams_in_view), rerr_cnt))
        return rval
...
class CameraCalibration(object):
    ...
    def addTargetView(self, rig_observations, T_tc_guess, force=False):
        ...
        if self.estimator_return_value.numIterations >= self.optimizerOptions.maxIterations:
            sm_python.logError("Did not converge in maxIterations... restarting...")
            raise OptimizationDiverged
        
        success = self.estimator_return_value.batchAccepted
        if success:
            sm_python.logDebug("The estimator accepted this batch")
            self.views.append(batch_problem)
        else:
            sm_python.logDebug("The estimator did not accept this batch")
        return success
...
def printParameters(cself, dest=sys.stdout):
    ...
    for bidx, baseline in enumerate(cself.baselines):
        T = sm_python.Transformation( baseline.T() )
...
def printDebugEnd(cself):
    ...
    for cidx, cam in enumerate(cself.cameras):
        ...
        if cidx>0:
            bidx=cidx-1
            T = sm_python.Transformation( cself.baselines[bidx].T() )
            for temp in T.t():
                print(temp)
...
def saveChainParametersYaml(cself, resultFile, graph):
    ...
    for bidx, baseline_dv in enumerate(cself.baselines):
        baseline = sm_python.Transformation( baseline_dv.T() )
        camNr = bidx+1
...
def plotCameraRig(baselines, fno=1, clearFigure=True, title=""):
    ...
    for baseline in baselines:
        temp.append(sm_python.Transformation(baseline.T()))
    baselines=temp
        
    #convert baselines to camera frames in coords of first camera
    camera_frames=list()
    camera_frames.append(sm_python.Transformation()) #first cam
    ...
    for camera_frame in camera_frames:
        sm_python.plotCoordinateFrame(a3d, camera_frame.T(), size=size)

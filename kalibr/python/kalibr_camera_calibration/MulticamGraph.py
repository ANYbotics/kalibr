...
class MulticamCalibrationGraph(object):
    ...
    def initializeGraphFromObsDb(self, obs_db):        
        ...
        t1 = time.time()
        total = t1-t0
        sm_python.logDebug("It took {0}s to build the graph.".format(total))
...
#############################################################    
    ...
    def getInitialGuesses(self, cameras):
        ...
        if not self.isGraphConnected():
            sm_python.logError("The cameras are not connected through mutual target observations! " 
        ...
        for baseline_edge_id in self.optimal_baseline_edges:
            ...
            if success:
                sm_python.logDebug("baseline_{0}_{1}={2}".format(camL_nr, camH_nr, baseline_HL.T()))
            else:
                sm_python.logError("initialization of camera pair ({0},{1}) failed  ".format(camL_nr, camH_nr))
                sm_python.logError("estimated baseline_{0}_{1}={2}".format(camL_nr, camH_nr, baseline_HL.T()))
        ...
        for baseline_id in range(0, self.numCams-1):
            ...
            baseline_HL = sm_python.Transformation()
        ...
        if not success:
            sm_python.logWarn("Full batch refinement failed!")
    ...
    def getTargetPoseGuess(self, timestamp, cameras, baselines_HL=[]):
        ...
        if not success:
            sm_python.logWarn("getTargetPoseGuess: solvePnP failed with solution: {0}".format(T_t_cN))
        
        #transform it back to cam0 (T_t_cN --> T_t_c0)
        T_cN_c0 = sm_python.Transformation()
    ...
    def getAllMutualObsBetweenTwoCams(self, camA_nr, camB_nr):
        ...
        try:
            ...
            sm_python.logError("getAllMutualObsBetweenTwoCams: no mutual observations between the two cams!")
            return [], []

...
import aslam_cv_backend_python as cvb
import sm_python
...
class CameraChainParameters(ParametersBase):
    ...
    def setExtrinsicsLastCamToHere(self, camNr, extrinsics):
        ...
        if not isinstance(extrinsics, sm_python.Transformation):
            raise RuntimeError("setExtrinsicsLastCamToHere(): provide extrinsics as an sm_python.Transformation object")
    ...
    def getExtrinsicsLastCamToHere(self, camNr):
        ...
        try:
            trafo = sm_python.Transformation( np.array(self.data["cam{0}".format(camNr)]['T_cn_cnm1']) )
    ...
    def setExtrinsicsImuToCam(self, camNr, extrinsics):
        if not isinstance(extrinsics, sm_python.Transformation):
            raise RuntimeError("setExtrinsicsImuToCam(): provide extrinsics as an sm_python.Transformation object")
    ...
    def getExtrinsicsImuToCam(self, camNr):      
        ...
        try:
            trafo = sm_python.Transformation( np.array(self.data["cam{0}".format(camNr)]['T_cam_imu']) )
            t = trafo.T() #for error checking
        except:

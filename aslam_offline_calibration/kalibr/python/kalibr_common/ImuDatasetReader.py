...
class BagImuDatasetReader(object):
    ...
    def sortByTime(self, indices):
        self.timestamp_corrector = sm_python.DoubleTimestampCorrector()
        timestamps=list()
    ...
    def truncateIndicesFromTime(self, indices, bag_from_to):
        ...
        if bag_from_to[0]<0.0:
            sm_python.logWarn("Bag start time of {0} s is smaller 0".format(bag_from_to[0]) )
        if bag_from_to[1]>baglength:
            sm_python.logWarn("Bag end time of {0} s is bigger than the total length of {1} s".format(bag_from_to[1], baglength) )
        ...
        sm_python.logWarn("BagImuDatasetReader: truncated {0} / {1} messages.".format(len(indices)-len(valid_indices), len(indices)))

...
import aslam_splines_python as asp
import aslam_backend_python as aopt
import aslam_bsplines_python
import kalibr_common as kc
import kalibr_errorterms as ket
from kalibr_imu_camera_calibration.IccCalibrator import CALIBRATION_GROUP_ID
from kalibr_imu_camera_calibration.IccCalibrator import HELPER_GROUP_ID
from kalibr_imu_camera_calibration.IccCalibrator import addSplineDesignVariables
...
class IccCamera():
    def __init__(self, camConfig, targetConfig, dataset, reprojectionSigma=1.0, showCorners=True, \
        ...
        self.T_extrinsic = sm_python.Transformation()
    ...
    def findOrientationPriorCameraToImu(self, imu):
        ...
        if problem.numErrorTerms() == 0:
            sm_python.logFatal("Failed to obtain orientation prior. "\
        ...
        self.T_extrinsic = sm_python.Transformation( sm_python.rt2Transform( R_i_c, self.T_extrinsic.t() ) )
    ...
    def findTimeshiftCameraImuPrior(self, imu, verbose=False):
        ...
        if len(omega_predicted_norm) == 0 or len(omega_measured_norm) == 0:
            sm_python.logFatal("The time ranges of the camera and IMU do not overlap. "\
                        "Please make sure that your sensors are synchronized correctly.")
            sys.exit(-1)
        ...
        if verbose:
            ...
            pl.show()
            sm_python.logDebug("discrete time shift: {0}".format(discrete_shift))
            sm_python.logDebug("cont. time shift: {0}".format(shift))
            sm_python.logDebug("dT: {0}".format(dT))
    ...
    def initPoseSplineFromCamera(self, splineOrder=6, poseKnotsPerSecond=100, timeOffsetPadding=0.02):
        ...
        pose = aslam_bsplines_python.BSplinePose(splineOrder, sm_python.RotationVector() )
    ...
    def addCameraErrorTerms(self, problem, poseSplineDv, T_cN_b, blakeZissermanDf=0.0, timeOffsetPadding=0.0):
        ...
        #progress bar
        iProgress = sm_python.Progress2( len(self.targetObservations) )
        iProgress.sample()
...
class IccCameraChain():
    ...
    def getResultBaseline(self, fromCamANr, toCamBNr):
        ...
        T_cB_cA = sm_python.Transformation( self.camList[idx].T_c_b_Dv.T() )
    ...
    def getResultTrafoImuToCam(self, camNr):
        ...
        T_c0_i = sm_python.Transformation( self.camList[0].T_c_b_Dv.T() )
        ...
        for cam in self.camList[1:camNr+1]:
            T_cNplus1_cN = sm_python.Transformation( cam.T_c_b_Dv.T() )
...
class IccImu(object):
    ...
    def updateImuConfig(self):
        self.imuConfig.setImuPose(sm_python.Transformation(sm_python.r2quat(self.q_i_b_Dv.toRotationMatrix()), \
    ...
    def loadImuData(self):
        ...
        # prepare progess bar
        iProgress = sm_python.Progress2( self.dataset.numMessages() )
        iProgress.sample()
        ...
        if len(self.imuData)>1:
            ...
            sm_python.logFatal("Could not find any IMU messages. Please check the dataset.")
            sys.exit(-1)
    ...
    def addAccelerometerErrorTerms(self, problem, poseSplineDv, g_w, mSigma=0.0, \
        ...
        #progress bar
        iProgress = sm_python.Progress2( len(self.imuData) )
        iProgress.sample()
    ...
    def initBiasSplines(self, poseSpline, splineOrder, biasKnotsPerSecond):
        ...
        self.gyroBias = aslam_bsplines_python.BSpline(splineOrder)
        self.gyroBias.initConstantSpline(start,end,knots, self.GyroBiasPrior )
        
        self.accelBias = aslam_bsplines_python.BSpline(splineOrder)
    ...
    def getTransformationFromBodyToImu(self):
        if self.isReferenceImu:
            return sm_python.Transformation()
        return sm_python.Transformation(sm_python.r2quat(self.q_i_b_Dv.toRotationMatrix()) , \
    ...
    def findOrientationPrior(self, referenceImu):
        ...
        angularVelocity = aslam_bsplines_python.BSpline(3)
        ...
        try:
            ...
            sm_python.logFatal("Failed to obtain initial guess for the relative orientation!")
            sys.exit(-1)
        ...
        if len(referenceAbsoluteOmega()) == 0 or len(absoluteOmega()) == 0:
            sm_python.logFatal("The time ranges of the IMUs published as topics {0} and {1} do not overlap. "\
        ...
        self.q_i_b_prior = sm_python.r2quat(q_i_b_Dv.toRotationMatrix())

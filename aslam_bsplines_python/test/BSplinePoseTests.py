...
class TestBSplinePose(unittest.TestCase):
    ...
    def testCurveToTransformation(self):
        rvs = (sm_python.RotationVector(), sm_python.EulerAnglesZYX(), sm_python.EulerRodriguez())
        for r in rvs:
            bsp = aslam_bsplines_python.BSplinePose(4,r)
    ...
    def testTransformationJacobian(self):
        ...
        for r in rvs:
            for order in range(2,7):
                bsp = aslam_bsplines_python.BSplinePose(order,r)
                ...
                for t in numpy.linspace(bsp.t_min(), bsp.t_max(), 4):
                    ...
                    JT = TJI[1]
                    J = numpy.dot(sm_python.boxMinus(numpy.dot(TJI[0],v)), JT)                    
        ...
        for r in rvs:
            for order in range(2,7):
                ...
                for t in numpy.linspace(bsp.t_min(), bsp.t_max(), 4):
                    ...
                    estJ = je(bsp.localCoefficientVector(t))
                    JT = CJI[1]
                    J = numpy.dot(sm_python.crossMx(numpy.dot(CJI[0],v)), JT)                    
    ...
    def testInitPose(self):
        bsp = aslam_bsplines_python.BSplinePose(4,sm_python.RotationVector())
    ...
    def testAngularVelocity(self):
        bsp = aslam_bsplines_python.BSplinePose(4,sm_python.EulerAnglesZYX())
        # Create two 

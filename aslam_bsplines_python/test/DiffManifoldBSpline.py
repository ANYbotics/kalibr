...
def createUniformKnotBSpline(order,segments,dim,knotSpacing=1.0):
    aspl = aslam_bsplines_python.EuclideanBSpline(order, dim)
...
class TestBSplines(BSplineTestCase):
    ...
    def test_init(self):
        ...
        v = (p_1 - p_0)/dt
        for order in range(2,10):
            aspl = aslam_bsplines_python.EuclideanBSpline(order, 3)
    ...
    def test_uniform(self):
        numpy.random.seed(1)
        for order in range(2,10):
            aspl = aslam_bsplines_python.EuclideanBSpline(order, 1)

...
boost::python::tuple timeInterval1(const aslam_bsplines::BSpline * bs)
{
  std::pair<double,double> ti = bs->timeInterval();
  return boost::python::make_tuple(ti.first,ti.second);
}

boost::python::tuple timeInterval2(const aslam_bsplines::BSpline * bs, int i)
{
...
boost::python::object getBiFunction(const aslam_bsplines::BSpline * bs, double t)
{
...
boost::python::object getCumulativeBiFunction(const aslam_bsplines::BSpline * bs, double t)
{
	typedef Eigen::CwiseNullaryOp <BiVector, Eigen::VectorXd> T;

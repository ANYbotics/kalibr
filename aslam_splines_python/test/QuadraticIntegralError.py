...
def createUniformKnotBSpline(order,segments,dim,knotSpacing=1.0):
    aspl = aslam_splines_python.OptEuclideanBSpline(order, dim)
...
def createUniformKnotOldBSpline(order,segments,dim,knotSpacing=1.0):
    aspl = aslam_bsplines_python.BSpline(order)
...
class TestQuadraticIntegralError(unittest.TestCase):
    def test_construction(self):
        ...
        sdv = aslam_splines_python.EuclideanBSplineDesignVariable(spline)
        ...
        for derivativeOrder in range(0, splineOrder - 1):
            p = aslam_backend_python.OptimizationProblem()
            for i in range(0, sdv.numDesignVariables()) : p.addDesignVariable(sdv.designVariable(i))
            aslam_splines_python.addQuadraticIntegralEuclideanExpressionErrorTermsToProblem(p, tMin, tMax, 100, lambda(time) : sdv.toEuclideanExpression(time, derivativeOrder), sqrtW)
            E = 0;

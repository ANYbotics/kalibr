...
#include <aslam_cameras/cameras/GridCalibrationTargetObservation.hpp>
#include <opencv2/calib3d/calib3d.hpp>
#include <sm_logging/logging.hpp>
...
class OmniProjection {
 public:
  ...
  OmniProjection(const sm::PropertyTree & config);
  ...
  template<typename DERIVED_K>
  bool isLiftable(const DERIVED_K & keypoint) const;
  ...
  bool isBinaryEqual(const OmniProjection<distortion_t> & rhs) const;
...
SM_BOOST_CLASS_VERSION_T1 (aslam::cameras::OmniProjection);

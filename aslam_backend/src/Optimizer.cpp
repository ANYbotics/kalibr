...
// M.inverse()
#include <Eigen/Dense>
#include <sm_eigen/assert_macros.hpp>
#include <aslam_sparse_block_matrix/linear_solver_dense.h>
#include <aslam_sparse_block_matrix/linear_solver_cholmod.h>

#ifndef QRSOLVER_DISABLED
#include <aslam_sparse_block_matrix/linear_solver_spqr.h>
#endif

#include <aslam_backend/backend/sparse_matrix_functions.hpp>
...
namespace aslam {
  namespace backend {
    ...
    {
      // \todo Add the remaining aslam_sparse_block_matrix solvers here.
      if (_options.linearSolver == "cholmod") {
        _options.verbose && std::cout << "Using the cholmod linear solver.\n";
        _solver.reset(new aslam_sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd>());
      } else if (_options.linearSolver == "dense") {
        _options.verbose && std::cout << "Using the dense linear solver.\n";
        _solver.reset(new aslam_sparse_block_matrix::LinearSolverDense<Eigen::MatrixXd>());
      }
#ifndef QRSOLVER_DISABLED
      else if (_options.linearSolver == "spqr") {
        _options.verbose && std::cout << "Using the SPQR linear solver.\n";
        _solver.reset(new aslam_sparse_block_matrix::LinearSolverQr<Eigen::MatrixXd>());
      }
#endif
      else {
                    ...
                    // Solve the dense system.
                              ...
                              if(!solutionSuccess)
                              {
                                ...
                                if(!_fallbackSolver) {  // initialise a new solver
                                  ...
                                  _fallbackSolver.reset(new aslam_sparse_block_matrix::LinearSolverQr<Eigen::MatrixXd>());
    ...
    void Optimizer::computeCovarianceBlocks(const std::vector<std::pair<int, int> > & blockIndices)
    {
      ...
      boost::shared_ptr<aslam_sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd> > solver(new aslam_sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd>());

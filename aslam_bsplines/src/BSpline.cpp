...
#include <aslam_sparse_block_matrix/linear_solver_cholmod.h>
#include <aslam_bsplines/BSpline.hpp>
#include <sm_common/assert_macros.hpp>
...
//      std::cout << "u:" << std::endl;
 //     std::cout << u << std::endl;
    ...
    void BSpline::initSplineSparseKnots(const Eigen::VectorXd &times, const Eigen::MatrixXd &interpolationPoints, const Eigen::VectorXd knots, double lambda)
    {
    	...
    	aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> A(rows,cols, true);
    	aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> b(rows,bcols, true);
    	...
    	aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> At(cols,rows, true);
    	aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> * Atp = &At;
    	A.transpose(Atp);
        
    	// A'b
    	aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> Ab(cols,bcols, true);
    	aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> * Abp = &Ab;
    	Atp->multiply(Abp, &b);
        
    	// A'A
    	aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> AtA(cols,cols, true);
    	aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> * AtAp = &AtA;
    	Atp->multiply(AtAp, &A);
        ...
        aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> Q(cols,cols,true);
        if (splineOrder_ == 2)
    	...
    	// solve:
    	aslam_sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd> solver;
    	solver.init();
...
Eigen::MatrixXd BSpline::segmentIntegral(int segmentIdx, const Eigen::MatrixXd & W, int derivativeOrder) const {
  ...
  return (svd.matrixU() * svd.singularValues().array().sqrt().matrix().asDiagonal()).transpose();
    ...
    aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> BSpline::curveQuadraticIntegralSparse(const  Eigen::MatrixXd & W, int derivativeOrder) const 
    {
        ...
        // create matrix:
        aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> Q_sparse(rows,cols,true);
        // place
    ...
    // sparse curveQuaddraticIntegral:
    aslam_sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd> BSpline::curveQuadraticIntegralDiagSparse(const Eigen::VectorXd & Wdiag, int derivativeOrder) const 
    {

...
config = sm_python.BoostPropertyTree()
# and parse the file
config.loadInfo(configFile);

# For comparison, we will create two undistortion pipelines
# This one does not undistort
nullProcessor = acv.NCameraPipeline( sm_python.PropertyTree(config,'NullPipeline') )
# this one does
undistortingProcessor = acv.NCameraPipeline( sm_python.PropertyTree( config, 'UndistortingPipeline') )

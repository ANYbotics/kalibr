...
#include <aslam_sparse_block_matrix/sparse_block_matrix.h>
...
template<typename M>
aslam_sparse_block_matrix::SparseBlockMatrix<M> buildRandomMatrix(Eigen::MatrixXi rows, Eigen::MatrixXi cols, double probabilityOfABlockBeingFilled) {
  typedef M matrix_t;

  aslam_sparse_block_matrix::SparseBlockMatrix<matrix_t> Mx(rows, cols);
...
TEST(sparse_block_matrixTestSuite, testBlock) {
  ...
  using namespace aslam_sparse_block_matrix;
  VectorXi rows(3);
  rows << 3, 6, 11;
  VectorXi cols(4);
  cols << 4, 9, 12, 18;
...
TEST(sparse_block_matrixTestSuite, testClone) {
  ...
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> ME = buildRandomMatrix<
      MatrixXd>(rows, cols, -1.0);

  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> * ME2 = ME.clone();
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> ME3;
  ME.cloneInto(ME3);

  {
    SCOPED_TRACE("");
    aslam_sparse_block_matrix::expectEqual(ME, ME3);
  }

  {
    SCOPED_TRACE("");
    aslam_sparse_block_matrix::expectEqual(ME, (*ME2));
  }

  delete ME2;

  // Create an not empty matrix.
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> M1 = buildRandomMatrix<
      MatrixXd>(rows, cols, 0.5);

  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> * M2 = M1.clone();
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> M3;
  M1.cloneInto(M3);

  {
    SCOPED_TRACE("");
    aslam_sparse_block_matrix::expectEqual(M1, (*M2));
  }

  {
    SCOPED_TRACE("");
    aslam_sparse_block_matrix::expectEqual(M1, M3);
  }
...
TEST(sparse_block_matrixTestSuite, testToDense) {
  ...
  {
    SCOPED_TRACE("");
    aslam_sparse_block_matrix::expectEqual(M1, D1);
  }
...
TEST(sparse_block_matrixTestSuite, testTranspose) {
  ...
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> * M1t = NULL;
  // First the NULL version.
  ASSERT_TRUE(M1.transpose(M1t));
  ASSERT_NE((aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> *)NULL, M1t);

  {
    SCOPED_TRACE("");
    aslam_sparse_block_matrix::expectEqual((*M1t), D1t);
  }
  {
    SCOPED_TRACE("");
    aslam_sparse_block_matrix::expectEqual(M1.transpose(), D1t);
  }

  // Now the non null version.
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> M2t(M1t->rowBlockIndices(),
                                                       M1t->colBlockIndices(),
                                                       true);
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> * M2tp = &M2t;
  ASSERT_TRUE(M1.transpose(M2tp));

  {
    SCOPED_TRACE("");
    aslam_sparse_block_matrix::expectEqual(M2t, D1t);
  }

  delete M1t;

  // What if there is cruff in the transpose matrix? This fails.
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> M3t = buildRandomMatrix<
      MatrixXd>(cols, rows, 0.5);
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> * M3tp = &M2t;
  ASSERT_TRUE(M1.transpose(M3tp));

  {
    SCOPED_TRACE("");
    // Fails.
    //aslam_sparse_block_matrix::expectEqual( M3t.toDense(), D1t);
  }
  ...
  rows << 2, 4, 6, 8;
  cols << 1, 2, 3;
  aslam_sparse_block_matrix::SparseBlockMatrix<Matrix<double, 2, 1> > M3 = buildRandomMatrix<Matrix<double, 2, 1> >(rows, cols, 0.5);
  {
    SCOPED_TRACE("");
...
TEST(sparse_block_matrixTestSuite, testScaleMatrix) {
  ...
  // Create an not empty matrix.
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> M1 = buildRandomMatrix<MatrixXd>(rows, cols, 0.5);
...
TEST(sparse_block_matrixTestSuite, testMatrixMatrixMultiplication) {
  ...
  // Create an not empty matrix.
  aslam_sparse_block_matrix::SparseBlockMatrix<MatrixXd> M2 = buildRandomMatrix<MatrixXd>(cols, rows, 0.5);

#include <sm_kinematics/EulerAnglesYawPitchRoll.hpp>

namespace sm { namespace kinematics {



  EulerAnglesYawPitchRoll::~EulerAnglesYawPitchRoll()
  {
    
  }

  Eigen::Matrix3d EulerAnglesYawPitchRoll::parametersToRotationMatrix(const Eigen::Vector3d & parameters, Eigen::Matrix3d * S) const
  {
    Eigen::Matrix3d C;
    ...
    Eigen::Vector3d EulerAnglesYawPitchRoll::rotationMatrixToParameters(const Eigen::Matrix3d & rotationMatrix) const
    {
    ...
    Eigen::Matrix3d EulerAnglesYawPitchRoll::parametersToSMatrix(const Eigen::Vector3d & parameters) const
    {
      double cy = cos(parameters[1]);
      double sy = sin(parameters[1]);
      double cz = cos(parameters[0]);
    ...
    Eigen::Matrix3d EulerAnglesYawPitchRoll::buildSMatrix(double sz, double cz, double sy, double cy) const
    {
      Eigen::Matrix3d S;
      S << 
	 0.0,  sz, -cy*cz,
    ...
    Eigen::Vector3d EulerAnglesYawPitchRoll::angularVelocityAndJacobian(const Eigen::Vector3d & p, const Eigen::Vector3d & pdot, Eigen::Matrix<double,3,6> * Jacobian) const
    {
      //double cx = cos(-p[2]);
      //double sx = sin(-p[2]);
      double cy = cos(p[1]);
